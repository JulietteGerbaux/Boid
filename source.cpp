...
int mouse_oldState = GLUT_UP;
int mouse_newState = GLUT_UP;
int window_w = 800;
int window_h = 600;
int mouse_moving = 0;
// Camera
...
void init(void)
{
	...
	// Init camera
	camera.init({ 0.0f,0.0f,0.0f }, 50.0f);
...
void mouseButton(int button, int state, int x, int y) {
	...
	//Add boid
	mouse_newState = mouse_buttons[GLUT_LEFT_BUTTON];
	if (mouse_newState == GLUT_UP && mouse_oldState == GLUT_DOWN) {
		if (mouse_moving == 0) flock.addBoid();
		mouse_moving = 0;
	}
